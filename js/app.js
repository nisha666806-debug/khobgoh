...
function dashboardPage(){
 ...
 $("content").innerHTML=`<div class="page-head"><div><h2>Dashboard</h2><p>Хулосаи ҳолати хобгоҳ</p></div></div><div class="stats-grid"><div class="stat-card"><b>${active}</b><small>Талабаҳо</small></div><div class="stat-card"><b>${sr.length}</b><small>Ҳуҷраҳо</small></div><div class="stat-card"><b>${occ}</b><small>Ҷойҳои ишғолшуда</small></div><div class="stat-card"><b>${total-occ}</b><small>Ҷойҳои холӣ</small></div><div class="stat-card"><b>${state.attendance.length}</b><small>Давомот</small></div><div class="stat-card"><b>${state.violations.length}</b><small>Қоидавайронкунӣ</small></div></div><div class="section-card"><h3>Ҳолати ҳуҷраҳо</h3><div class="room-grid">${sr.map(r=>`<div class="room-mini" onclick="openRoom('${r.id}')"><b>${esc(r.name)}</b><strong>${state.students.filter(s=>s.roomId===r.id&&s.status==="Active").length}/${cap(r)}</strong></div>`).join("")}</div></div><div class="section-card"><h3>Ҷадвали ҳафтаинаи фаъолиятҳо</h3>${renderActivityTable()}</div><div class="section-card"><h3>Тамоюли 6 моҳи охир</h3><div class="chart-box"><canvas id="trendChart"></canvas></div></div>`;
 renderTrendChart();
}
...
function studentsPage(){
 ...
 $("content").innerHTML=`<div class="page-head"><div><h2>Талабаҳо</h2><p>Рӯйхати донишҷӯён</p></div>${isManager()?'<button class="btn btn-primary" onclick="openAddStudent()">+ Талабаи нав</button>':""}</div><div class="toolbar"><input id="studentSearch" placeholder="Ҷустуҷӯ..." oninput="renderStudentRows(this.value)"></div><div class="table-wrap"><table><thead><tr><th>Ном</th><th>Синф</th><th>Телефон</th><th>Ҳуҷра</th><th>Ҷой</th><th>Мураббӣ</th><th>Ҳолат</th></tr></thead><tbody id="studentRows"></tbody></table></div>`;renderStudentRows("");
}
...
function openAddStudent(){modal(`<h2>Талабаи нав</h2><div class="form-grid"><div class="form-group"><label>Номи пурра</label><input id="studentName"></div><div class="form-group"><label>Таваллуд</label><input id="studentDob" type="date"></div><div class="form-group"><label>Телефон</label><input id="studentPhone"></div><div class="form-group"><label>Телефони падар</label><input id="studentFather"></div><div class="form-group"><label>Суроға</label><input id="studentAddress"></div><div class="form-group"><label>Синф</label><input id="studentClass"></div><div class="form-group"><label>Муаллими синф</label><input id="studentTeacher"></div></div><button class="btn btn-primary" onclick="createStudent()">Сабт кардан</button>`)}
async function createStudent(){const name=$("studentName").value.trim();if(!name)return err("Номро ворид кунед.");await addDoc(collection(db,"students"),{name,dob:$("studentDob").value,phone:$("studentPhone").value.trim(),fatherPhone:$("studentFather").value.trim(),address:$("studentAddress").value.trim(),className:$("studentClass").value.trim(),classTeacher:$("studentTeacher").value.trim(),supervisorId:null,roomId:null,place:null,status:"Active",createdAt:serverTimestamp(),updatedAt:serverTimestamp()});closeModal();ok("Талаба илова шуд.")}
...
async function deleteStudent(id){if(!confirm("Талаба пурра нест карда шавад? Ин амал бозгашт надорад."))return;await deleteDoc(doc(db,"students",id));closeModal();ok("Талаба нест карда шуд.")}
